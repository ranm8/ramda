...
var mergeWith = require('../src/mergeWith');
...
describe('mergeWith', function() {
  ...
  it('takes two objects, merges their own properties and returns a new object', function() {
    var a = {w: 1, x: 2};
    var b = {y: 3, z: 4};
    eq(mergeWith(last, a, b), {w: 1, x: 2, y: 3, z: 4});
  });
  ...
  it('applies the provided function to the value from the first object and the' +
    ...
    var a = {x: 'a', y: 'c'};
    var b = {x: 'b', z: 'd'};
    var c = mergeWith(function(a, b) { return a + b; }, a, b);
    eq(c, {x: 'ab', y: 'c', z: 'd'});
  });

  it('is not destructive', function() {
    var a = {w: 1, x: 2};
    var res = mergeWith(last, a, {x: 5});
  ...
  it('reports only own properties', function() {
    function Cla() {}
    Cla.prototype.x = 5;
    eq(mergeWith(last, new Cla(), {w: 1, x: 2}), {w: 1, x: 2});
    eq(mergeWith(last, {w: 1, x: 2}, new Cla()), {w: 1, x: 2});
    eq(mergeWith(last, new Cla(), {w: 1}), {w: 1});
  });

  it('is curried', function() {
    eq(mergeWith(last)({w: 1, x: 2})({y: 3, z: 4}), {w: 1, x: 2, y: 3, z: 4});
  });
});
